...
static void IRAM_ATTR uart_rev_isr(void *arg)
{
    ...
    // }
    uint32_t uart_intr_status = uart->int_st.val;
    uart->int_clr.val = uart_intr_status; // clear all
    // 读出收到的数据
    ...
    if (uart->int_st.txfifo_empty_int_st)
    {
        ...
        // {
        uart->int_ena.txfifo_empty_int_ena = 0;
        static DRAM_ATTR uint8_t  data = 0xfe;
        ...
        if (ret == 0)
        {
            ...
            uart->int_clr.txfifo_empty_int_clr = 1;
            uart->int_ena.txfifo_empty_int_ena = 1;
        }
        else
        {
            uart->int_clr.txfifo_empty_int_clr = 1;
...
void serial_init(void)
{
    ...
    // context.uart_hw = UART_LL_GET_HW(UART_NUM);
    ...
    const uart_config_t uart_config = {.baud_rate = CONFIG_SERIAL_BAUD,
    ...
    ESP_ERROR_CHECK(esp_intr_alloc(uart_periph_signal[UART_NUM].irq, 0, (void (*)(void *))uart_rev_isr,
                       &UART0, &intr_handle));
