...
void __always_inline IRAM_ATTR udp_send_data(const void *data, uint16_t size)
{
    struct pbuf *out;
	out = pbuf_alloc(PBUF_TRANSPORT, size, PBUF_POOL);
	if (out){
    ...
    for (;;)
    {
        if (0 != ulTaskNotifyTake(pdTRUE, portMAX_DELAY))
        {
            if (send_len > 0)
            {
                ...
                static DRAM_ATTR char buffer[1024];
                int len = send_len;
                memcpy(buffer, send_buffer, len);
                send_len = 0;
                taskEXIT_CRITICAL(&my_spinlock);
                udp_send_data(buffer, len);
            }
            // else{
            //     log_i("no send: len: %d----------------\n\n", send_len);
            // }
...
void wifi_task() {
  static DRAM_ATTR uint8_t data = 0x7e;
  int ret = -1;
  do{
